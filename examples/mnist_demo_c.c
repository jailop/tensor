...
void mnist_network_destroy(MNISTNetwork* net) {
    ...
    if (net->softmax)
        layer_softmax_destroy(net->softmax);
    ...
    if (net->predictions)
        matrix_float_destroy(net->predictions);
    ...
    if (net->grad_input)
        matrix_float_destroy(net->grad_input);
    /* Clear structure */
    memset(net, 0, sizeof(MNISTNetwork));
}

/**
 * Create and initialize the MNIST network
 */
int mnist_network_create(MNISTNetwork* net) {
    ...
    for (int i = 0; i < NUM_LINEAR_LAYERS; i++) {
        bool res = layer_linear_create_float(layer_sizes[i], layer_sizes[i+1],
                true, &net->fc[i]); 
        if (res != TENSOR_SUCCESS) {
...
int mnist_network_backward(MNISTNetwork* net, MatrixFloatHandle grad_output) {
    ...
    for (int i = 0; i < NUM_LINEAR_LAYERS; i++) {
        if (net->grad_h[i])
            matrix_float_destroy(net->grad_h[i]);
    }
    for (int i = 0; i < NUM_RELU_LAYERS; i++) {
        if (net->grad_a[i])
            matrix_float_destroy(net->grad_a[i]);
    }
    if (net->grad_input)
        matrix_float_destroy(net->grad_input);
    ...
    for (int i = NUM_RELU_LAYERS - 1; i >= 0; i--) {
        layer_relu_backward_float(net->relu[i], net->grad_a[i],
                &net->grad_h[i]);
        if (i > 0) {
            layer_linear_backward_float(net->fc[i], net->grad_h[i],
                    &net->grad_a[i-1]);
        } else {
            layer_linear_backward_float(net->fc[i], net->grad_h[i],
                    &net->grad_input);
...
/**
 * Read 32-bit big-endian integer from file
 */
...
/**
 * Load MNIST images directly into a Tensor (matrix)
 ...
 */
int load_mnist_images(const char* filename, MatrixFloatHandle* images_tensor,
        size_t* num_images) {
    ...
    if (magic != 2051) {
        fprintf(stderr,
                "Error: Invalid MNIST image file (magic number mismatch)\n");
        fclose(file);
        return 0;
    }
 ...
 */
int load_mnist_labels(const char* filename, uint8_t** labels,
        size_t* num_labels) {
    FILE* file = fopen(filename, "rb");
    if (!file) {
        fprintf(stderr, "Error: Cannot open file %s\n", filename);
    ...
    if (magic != 2049) {
        fprintf(stderr,
                "Error: Invalid MNIST label file (magic number mismatch)\n");
        fclose(file);
        return 0;
    }
...
int main(int argc, char* argv[]) {
    ...
    snprintf(train_images_path, sizeof(train_images_path),
            "%strain-images-idx3-ubyte", data_path);
    snprintf(train_labels_path, sizeof(train_labels_path),
            "%strain-labels-idx1-ubyte", data_path);
    snprintf(test_images_path, sizeof(test_images_path),
            "%st10k-images-idx3-ubyte", data_path);
    snprintf(test_labels_path, sizeof(test_labels_path),
            "%st10k-labels-idx1-ubyte", data_path);

    /* Load training data */
    MatrixFloatHandle train_images_tensor = NULL;
    uint8_t* train_labels = NULL;
    size_t num_train_images, num_train_labels;
    int res = load_mnist_images(train_images_path, &train_images_tensor,
            &num_train_images);
    if (!res) {
        return 1;
    }
    res = load_mnist_labels(train_labels_path, &train_labels,
            &num_train_labels);
    if (!res) {
    ...
    res = load_mnist_images(test_images_path, &test_images_tensor,
            &num_test_images);
    if (!res) {
        matrix_float_destroy(train_images_tensor);
        free(train_labels);
    ...
    res = load_mnist_labels(test_labels_path, &test_labels, &num_test_labels);
    if (!res) {
        matrix_float_destroy(train_images_tensor);
        free(train_labels);
    ...
    float loss, acc; 
    size_t rows, cols, g_rows, g_cols;
    const float* diff_ptr;
    const float* grad_ptr;
    MatrixFloatHandle grad_output, grad_w;
    for (epoch = 0; epoch < MAX_EPOCHS; ++epoch) {
        ...
        for (size_t batch = 0; batch < num_batches; ++batch) {
            ...
            matrix_float_submatrix(train_images_tensor, start_idx,
                    start_idx + BATCH_SIZE, 0, IMAGE_PIXELS, &batch_input);
            ...
            matrix_float_cross_entropy_loss(net.predictions, batch_targets,
                    &loss);
            matrix_float_compute_accuracy(net.predictions,
                    &train_labels[start_idx], BATCH_SIZE, &acc);
            epoch_loss += loss;
            epoch_accuracy += acc;
    ...
    for (size_t batch = 0; batch < num_test_batches; ++batch) {
        ...
        matrix_float_submatrix(test_images_tensor, start_idx,
        ...
        float acc;
        matrix_float_compute_accuracy(net.predictions, &test_labels[start_idx],
                test_batch_size, &acc);
        test_accuracy += acc;
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        matrix_float_submatrix(test_images_tensor, i, i + 1, 0, IMAGE_PIXELS,
                &sample_input);
        ...
        printf("Image %zu: True label = %d, Predicted = %zu, "
               "Confidence = %.2f%%\n",
